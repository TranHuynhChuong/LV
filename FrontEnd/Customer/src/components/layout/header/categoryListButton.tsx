...
export function CategoryTreeNodes({ nodes }: { nodes: CategoryNode[] }) {
  ...
  return (
    <div className="space-y-1">
      {nodes.map((node) => (
        <div key={node.id} className="my-2 pl-4 pt-2 text-sm border-t-1">
        ...
        <ScrollArea className="h-90">
          <div className="flex flex-wrap space-y-5  p-6 h-fit">
            {nodes.map((node) => (
              <div
                key={node.id}
                className="min-w-42 max-w-fit flex-1/3 lg:flex-1/4 xl:flex-1  px-5"
              >
...
export function CategoryTreeMenuOnMobile({ nodes }: { nodes: CategoryNode[] }) {
...
export default function CategoryList() {
  ...
  useEffect(() => {
    api
      .get('/categories')
      .then((res) => {
        const categoriesMapped = (res.data as ApiCategory[]).map((cat) => ({
          id: cat.TL_id,
          name: cat.TL_ten,
          parent: (res.data as ApiCategory[]).find((c) => c.TL_id === cat.TL_idTL)?.TL_ten || '',
        }));
