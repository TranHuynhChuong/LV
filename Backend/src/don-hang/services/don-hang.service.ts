...
import { DanhGiaServiceUtil } from '../../danh-gia/services/danh-gia.service';
import {
  BadRequestException,
  ConflictException,
  Injectable,
...
import { MaGiamUtilService } from 'src/ma-giam/services/ma-giam.service';
import { NhanVienUtilService } from 'src/nguoi-dung/nhan-vien/services/nhan-vien.service';
import { SachUtilService } from 'src/sach/services/sach.service';
import { TTNhanHangDHService } from '../../tt-nhan-hang/services/tt-nhan-hang.service';
...
import { KhachHangUtilService } from 'src/nguoi-dung/khach-hang/services/khach-hang.service';
...
import { DiaChiService } from 'src/dia-chi/services/dia-chi.service';
...
export class DonHangService {
      ...
      await session.withTransaction(async () => {
        ...
        if (data.PhuongThucThanhToan === 'ZaloPay' && data.DH.KH_id) {
          try {
            ...
            const result = await this.ThanhToanService.create(
              orderId,
              transId,
              total,
              data.DH.KH_id,
  ...
  async findById(id: string, filterType?: OrderStatus): Promise<any> {
    ...
    const payment = await this.ThanhToanService.queryOrder(result.DH_id);
